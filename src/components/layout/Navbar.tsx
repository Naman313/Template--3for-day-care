...
export const Navbar: React.FC<NavbarProps> = ({ onOpenTourModal }) => {
  ...
  const isLightPage = ['/our-learning-approach', '/child-safety', '/resources', '/faq'].includes(location.pathname);
    ...
    <header
      className={`fixed top-0 left-0 right-0 z-50 w-full transition-all duration-300 ${
        scrolled
          ...
          : isLightPage
          ? 'bg-[#FFFDF7]/95 backdrop-blur-md border-b border-[#FAF0CA] py-3 sm:py-4 shadow-sm'
    ...
    >
      <div className="w-full max-w-[1600px] mx-auto flex items-center justify-between gap-4">
        ...
        <Link to="/" className="flex items-center gap-3 group flex-shrink-0">
          ...
          />
          <div className="hidden md:flex flex-col font-extrabold text-[10px] sm:text-[11px] leading-tight tracking-wide whitespace-nowrap">
          ...
          <nav className="flex items-center gap-5 xl:gap-8 flex-shrink-0">
            {navLinks.map((link) => {
              ...
              const linkTextColor = scrolled
                ? 'text-white hover:text-[#FAF0CA]'
                : isLightPage
                ? 'text-[#0D3B66] hover:text-[#F95738]'
                : 'text-white hover:text-[#FAF0CA]';
              ...
              return (
                <Link
                  key={link.path}
                  to={link.path}
                  className={`whitespace-nowrap text-xs lg:text-sm xl:text-base font-poppins font-bold tracking-tight transition-all duration-200 ${linkTextColor} ${
